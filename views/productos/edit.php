        ...
        <div class="flex flex-col bg-white px-6 py-8 rounded-lg shadow-md">
            ...
            <div class="flex flex-col md:flex-row mt-5">
                <div class="md:w-3/12 md:mr-3">
                    ...
                    <select v-model="item.unidad_id"
                            class="bg-white pr-2 border border-gray-300 text-gray-900 sm:text-base rounded-lg focus:outline-none focus:border-cyan-600 focus:ring-cyan-600 focus:ring-2 block w-full p-2.5">
                        <option v-for="unidad in unidades" :value="unidad.id">{{ unidad.codigo }}</option>
                    </select>
                </div>
                <div class="md:w-3/12 md:mr-3">
            ...
            <div class="flex justify-end mt-8">
                ...
                <button_ color="cyan" @click="updateItem()"><i class="fa fa-save pr-1"></i> Guardar</button_>
            </div>
        </div>
    </div>
</div>
    ...
    const app = createApp({
        ...
        methods: {
            loadItem: function (){
                ...
                var porciones = url.pathname.split('/');
                var getid = porciones[3];
                axios.post('/api/productos/getid/' + getid).then(res => {
